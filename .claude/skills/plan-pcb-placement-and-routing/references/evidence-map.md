...
| `edge_floor_fallback` | #975. Per ref, a declared edge connector this run seated with pad copper inside the board-edge floor because no pose the seat ladder tried at its rotation clears it — that seat moved inward, and later rungs only when the shortfall faces a side moving inward cannot fix: `pose`, `required_mm`, the worst `shortfall_mm` and `min_gap_mm`, `n_pads_short` with up to four `pads` (`pad_ref`, `pad_index`, `side`, `gap_mm`, `shortfall_mm`; `gap_mm` is null on a sampled outline), the pads that could not be measured, `kept` (`conflict_free`, or `crowding` when no seat cleared the placed parts), and `why` it stayed: `band_min` (clearing it goes under the declared overhang minimum), `setback`, `along_edge`, `outline_sampled`, `refused`, `crowds`, `still_short`, `nearest_edge` (the clearing pose reads nearest another edge), `crowding`. Filled on a fresh seed and on `--repair`; `{}` on `--reseat`, which drops its scope's edge declarations. Empty means every edge seat this run made clears the floor on the pads it could measure — NOT that `pad_edge_after` is clean, which also grades parts no seat moved. Join to `pad_edge_after.findings` by `pad_ref` + `pad_index`. Never changes the exit code |
