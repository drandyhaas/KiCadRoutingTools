...
| `power_widths.<net>.under_mm` / `under_share` | **How MUCH of it did not happen** (#1033). The bottleneck above is one number; this is the length. Measured on the board the run wrote, per `--power-nets-widths` net: `requested_mm`, `length_mm`, `under_mm`, `min_mm`, `by_width_mm`. Run 32 asked +3V3 for 0.3 and shipped 34% of its length at the 0.127 signal width with the bottleneck reading the same as a 1 mm neck would. A post-route pass widens power copper back wherever the width fits on exact geometry, so these numbers are what SHIPS. `design_rules.narrowed` carries ONE `track_width` row per power net that still ships under its width (site `power copper shipped under width`, with `length_mm` = `under_mm`), replacing the per-attempt rows, so `--strict-sizes` exits 3 on a power net only when shipped copper is under width -- and only for the power nets THIS step routed (`power_widths.<net>.in_run_scope`, list in `power_widths_run_scope`); an earlier step's narrow copper on another power net is disclosed, not failed. `power_widths_measured_on` says which copper was measured (the written board on the CLI; the change-set on the GUI, before any post-apply oracle leg). The grader twin is `board_score --net-min-widths` `components.net_widths.nets.<net>.length_under_mm` |
