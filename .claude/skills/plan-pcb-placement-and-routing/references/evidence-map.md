...
| `pad_conflicts_unseated` / `pad_conflicts_unseated_pairs` | #982. Real copper, named, and NOT charged to the seed: a pair against a part it could not seat, which was written at the pose it came in with. Whether anything lands there is incidental to this seed — the count moved 0 → 1 on ulx3s when an unrelated connector was seated 0.386 mm inward. **Do not read a non-zero value as a placement regression, and do not read zero as a clean board**: the exit code is already 4 for the unseated part, and `check_assembly` grades that board NOT BUILDABLE. Seat the part and these go with it |
| `pad_conflicts_inherited` | The board's own: neither side is a part this seed moved. Reported, never charged |
| `pad_conflicts_after` | The written board's total, which those three buckets PARTITION. Check the arithmetic rather than trusting one bucket |
