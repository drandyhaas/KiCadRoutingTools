...
| `placement.terms.<t>.basis` | the POPULATION a term was measured over. When it moves between laps the term is **not judged** -- a total over a different population is not a larger or smaller version of the first (the argument `commensurability` makes about `blocking`). Measured: a plane term's blocker set went 3 -> 11 -> 3 as parts were frozen and its value went 6.8 -> 101.4 -> 0.0 with it |
