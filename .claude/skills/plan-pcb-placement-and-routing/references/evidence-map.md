...
| `connector_requirements.unmeasured[].requirement` / `.reason` / `.graded_on` | What was NOT measured, per ref: `presence` (not on the board), `overhang_body` (no measurable drawn body — the band was graded on the old occupancy reading named in `graded_on`), `pad_copper_outside` (copper past the outline went ungraded: a pad shape the edge grader cannot model, or an entry claiming an edge whose body could not be read — whatever its band says), `center_on_edge` / `along_edge_band` (abstained, or never measured). A ref can be here AND in the errors |
