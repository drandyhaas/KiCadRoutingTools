...
| one part in the wrong place, and you know where it belongs | `place_pose` — set, rotate, face or lock; it grades the pose and refuses one that makes the board's placement legality worse (pad clearance, pads and footprint graphic copper against the outline; its summary's `legal_scope` / `legal_unmeasured` say what it does not grade) | a whole-board search, which orders violators by its own priority and may never reach yours |
