...
- **Inner-layer launch via escape vias** - When the stub layer's launch corridor is jammed (a fanout escaping into a dense pad field) but the terminal carries a through-via/THT barrel, the coupled pair launches on an open inner/back layer the barrel spans instead of the stub layer (issue #195). Lets under-pad fanout (`--escape-method underpad`) compose with coupled routing; also tries launching toward the other terminal when the stub faces away from the route
- **Hybrid escape (direct coupled middle + point-to-point legs)** - Last-resort fallback for terminals with no clean connector join (the straight connectors would graze the partner's escape via, the pair must swap sides, or no valid setback exists). Routes the coupled centerline *straight* from source to target on the best open layer (no setback/connector/polarity stage), offsets it into a parallel P/N pair, then attaches each terminal with a short single-ended A* leg that drops its own escape via and routes around the partner copper — so polarity is resolved at the pads, not by a coupled stage. The P/N side is chosen to minimise leg crossings. Yields a coupled run with single-ended only on the last millimetre at each terminal. Needs the terminal to have room for an escape via (through-via/THT, or a bare pad with clearance) — a terminal physically boxed by a surface fanout needs under-pad escape or a placement change instead. Gated by `diff_pair_hybrid_escape` (default on); returns the pair to the single-ended follow-up when it can't lay a clean route
