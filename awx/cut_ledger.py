...
def main():
    ...
    a = ap.parse_args()
    # THE DESIGN RULES, from the board (rules.py). The locals above are
    # ALSO re-read explicitly: install does reach a module running as
    # '__main__' (it matches on __file__), but this file's TRACK / CLEAR /
    # NEED are its own names copied from braid, and re-reading them here is
    # the spelling that cannot go stale if that matching ever changes.
    ...
    c.offsets(0.35)
    c.reserve_intervals()
    # `pages=` was dropped from Schedule on take5 (the page assignment is
    # the level-5 MILP's now, not a constructor argument)
    sched = Schedule(c.launch, c.target, ctx.tooth_layer,
                     dest_layer=ctx.dest_layer)
    # take5 folded plan_columns into lay_lanes, which now takes the
    # schedule itself and derives the columns, the required-layer
    # intervals and the diver windows from it
    c.lay_lanes(sched)
    sp = c.spine
