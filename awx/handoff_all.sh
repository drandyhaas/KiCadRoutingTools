...
for K in "$@"; do
  echo "=== K$K"
  python3 make_handoff.py "ho_k${K}.kicad_pcb" "$K" 2>&1 | tail -1
  VIEW=$(python3 view_of.py "$K")
  python3 render_eco.py "ho_k${K}.kicad_pcb" \
    "$DEST/take4_k${K}_handoff.png" \
    --nets "$(python3 coherent_nets.py "$K")" --view "$VIEW" 2>&1 \
    | sed 's/highlighted.*//'
  cp "ho_k${K}.kicad_pcb" "$DEST/take4_k${K}_handoff.kicad_pcb"
  [ -f "ho_k${K}.kicad_pro" ] && cp "ho_k${K}.kicad_pro" \
