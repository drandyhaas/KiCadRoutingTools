...
for K in "$@"; do
  echo "=== K$K  $(date +%H:%M:%S)"
  python3 make_overlay.py "tmp/ov_k${K}.kicad_pcb" "$K" 2>&1 | tail -2
  VIEW=$(python3 view_of.py "$K")
  python3 render_eco.py "tmp/ov_k${K}.kicad_pcb" "$DEST/take4_k${K}_intent.png" \
    --nets "$(python3 coherent_nets.py "$K")" --view "$VIEW" 2>&1 | tail -1
  cp "tmp/ov_k${K}.kicad_pcb" "$DEST/take4_k${K}_intent.kicad_pcb"
  [ -f tmp/ov_k${K}.kicad_pro ] && cp "tmp/tmp/ov_k${K}.kicad_pro" \
