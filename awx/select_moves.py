    ...
    def move(self, m: Move) -> Move:
        ...
        if mm is None:
            mm = Move(net=m.net, kind=m.kind,
                      ...
                      # the climb travels with the move too: without it the
                      # mirrored board's conflict test took the single-span
                      # path and a board and its mirror ran different
...
def _select(menu: Dict[str, List[Move]],
    ...
    def cost(n: str, m: Move) -> float:
        ...
        chan = _length(m)
        return via_weight * m.vias + channel_weight * chan + reach
