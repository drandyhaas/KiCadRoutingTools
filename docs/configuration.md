...
| `--strict-sizes` | off | Exit 3 when any feature was delivered below its requested size or a fab-tier escalation fired. For `--power-nets` track widths this means copper that SHIPS under width after the post-route widen pass (one `design_rules` row per power net, #1033), not every narrower routing attempt |
