...
1. **Polarity swap** - Off by default; allow it per pair with `--polarity-swap-nets <patterns>` (`'*'` = every pair; the GUI's "Polarity-swap allowed nets" field, which is EMPTY by default = deny all, matching the CLI). Only allow pairs an endpoint can compensate (FPGA generic I/O, polarity-tolerant SerDes) - never USB/MIPI/TMDS (#279). For a denied pair, a mismatch is re-routed with the connectors out the opposite side at one end (bare-pad endpoints only), and skipped with a warning if that is not possible; denied-but-wanted swaps are listed in `polarity_swap_denied_pairs` in the JSON summary.
   The **2-terminal hybrid escape** (coupled middle + point-to-point terminal legs) can also produce a swap (#266): when the pair is SIDE-FLIPPED - the P pad on opposite sides of the coupled centerline at the two ends - one terminal leg otherwise has to wrap around its partner. A permitted pair gets a target-pad swap when the swapped candidate is the one the hybrid's own selector prefers (fewest intra-pair overlaps, then via-weighted length, first-on-tie). That comparison is LOCAL to one pair and one layer combination: the pad rename is permanent and changes the board every later pair sees, so a swap that wins its own candidate list can still leave the finished board with more copper (measured on `glasgow_revC`: +13.6 mm over 14 pairs). A pair that stays side-flipped without a swap is named in `polarity_flip_unswapped_pairs` in the JSON summary (disclosure only - the copper is legal and connected, just longer and less symmetric); that disclosure is recorded from all three hybrid commit sites (first-pass, multipoint leg, reroute), only the first of which can actually apply a swap. The multipoint leg loop does not swap yet
