...
def board_path_for_analysis(board_filename):
    ...
    Inside KiCad the user edits an in-memory board that may be newer than
    the file on disk, and the routing dialog is modal so they can't save
    first. The live board is snapshotted to a temp file via the IPC
    adapter (kipy Board.save_as -> SaveCopyOfDocument: the open document
    keeps its own file association, the user's file is never touched) and
    the analysis runs on that.

    Returns the path to analyze, or None if the run should not proceed.
    """
    adapter = None
    # KiCad sets KICAD_API_SOCKET when it spawns an IPC plugin - without it
    # we're in tests / CLI (even if kipy happens to be installed): use the file.
    ...
    if adapter is not None:
        ...
        try:
            # save_as(include_project=False): the analysis snapshot carries
            # copper only, and never touches the user's own open file.
            adapter.save_board_snapshot(snapshot)
            # Stage the on-disk project siblings beside the snapshot so the
            # analysis sees the real netclasses and layer rules (#498) -- the
            # snapshot itself carries no project. The snapshot path is
            # deterministic, so a sibling left by an earlier snapshot of a
            ...
            pass  # kipy not installed: not running inside KiCad - use the file
