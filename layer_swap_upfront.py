"""
Upfront layer swap optimization for PCB routing.
...
def optimize_diff_pair_layers_upfront(
    ...
    diff_pair_ids_to_route_set: List[Tuple[str, any]],  # [(pair_name, pair), ...]
    diff_pairs: Dict[str, any],  # all diff pairs dict
    can_swap_to_top_layer: bool,
    all_swap_vias: List,
    all_segment_modifications: List
) -> int:
    """
    Perform upfront layer swap optimization for differential pairs.

    Analyzes all diff pairs and applies beneficial swaps BEFORE MPS ordering,
    so ordering sees correct segment layers.

    Args:
        pcb_data: PCB data structure
        config: Routing configuration
        diff_pair_ids_to_route_set: List of (pair_name, pair) tuples to route
        diff_pairs: Dict of all differential pairs
        can_swap_to_top_layer: Whether swaps to F.Cu are allowed
        all_swap_vias: Output list to append vias to
        all_segment_modifications: Output list to append modifications to

    Returns:
        Total number of layer swaps applied
    """
    ...
    from stub_layer_switching import (
        ...
        collect_stub_endpoints_by_layer, validate_swap
    )
...
def optimize_single_ended_layers_upfront(
    pcb_data: PCBData,
    config: GridRouteConfig,
    net_ids: List[Tuple[str, int]],  # [(net_name, net_id), ...]
    ...
    """
    Perform upfront layer swap optimization for single-ended nets.

    Args:
        pcb_data: PCB data structure
        config: Routing configuration
        net_ids: List of (net_name, net_id) tuples
        diff_pair_net_ids: Set of net IDs that belong to diff pairs (to exclude)
        can_swap_to_top_layer: Whether swaps to F.Cu are allowed
        all_swap_vias: Output list to append vias to
        all_segment_modifications: Output list to append modifications to
        all_stubs_by_layer: Optional dict of diff pair stubs by layer (for combined validation)
        verbose: Whether to print detailed messages

    Returns:
        Total number of layer swaps applied
    """
    # Identify single-ended nets
    single_ended_net_ids = [(name, nid) for name, nid in net_ids if nid not in diff_pair_net_ids]

    if not single_ended_net_ids:
        return 0

    from stub_layer_switching import (
        get_stub_info, apply_stub_layer_switch, validate_single_swap,
    ...
    for net1_name, (src1, tgt1, sources1, targets1, net1_id) in single_net_layer_info.items():
        ...
        for net2_name, (src2, tgt2, sources2, targets2, net2_id) in single_net_layer_info.items():
            ...
            if src1 == tgt2 and tgt1 == src2:
                ...
                # Try Option 1: Both move to layer tgt1 (=src2)
                # Net1 source: src1 -> tgt1, Net2 target: tgt2 (=src1) -> tgt1
                opt1_valid = False
                if src1_stub and tgt2_stub:
                    # Check can_swap_to_top_layer restriction
                    if can_swap_to_top_layer or tgt1 != 'F.Cu':
                        v1, _ = validate_single_swap(src1_stub, tgt1, combined_stubs_by_layer, pcb_data, config)
                        v2, _ = validate_single_swap(tgt2_stub, tgt1, combined_stubs_by_layer, pcb_data, config)
                        opt1_valid = v1 and v2

                # Try Option 2: Both move to layer src1 (=tgt2)
                # Net1 target: tgt1 -> src1, Net2 source: src2 (=tgt1) -> src1
                opt2_valid = False
                if tgt1_stub and src2_stub:
                    if can_swap_to_top_layer or src1 != 'F.Cu':
                        v1, _ = validate_single_swap(tgt1_stub, src1, combined_stubs_by_layer, pcb_data, config)
                        v2, _ = validate_single_swap(src2_stub, src1, combined_stubs_by_layer, pcb_data, config)
                        opt2_valid = v1 and v2

                if opt1_valid:
                    # Apply Option 1
                    vias1, mods1 = apply_stub_layer_switch(pcb_data, src1_stub, tgt1, config, debug=False)
                    vias2, mods2 = apply_stub_layer_switch(pcb_data, tgt2_stub, tgt1, config, debug=False)
                    ...
                    swap_pair_count += 1
                    total_layer_swaps += 2
                    print(f"  Swap pair: {net1_name} ({src1}->{tgt1}) + {net2_name} ({tgt2}->{tgt1})")
                    break
                elif opt2_valid:
                    # Apply Option 2
                    vias1, mods1 = apply_stub_layer_switch(pcb_data, tgt1_stub, src1, config, debug=False)
                    vias2, mods2 = apply_stub_layer_switch(pcb_data, src2_stub, src1, config, debug=False)
                    all_swap_vias.extend(vias1 + vias2)
                    all_segment_modifications.extend(mods1 + mods2)
                    applied_single_swaps.add(net1_name)
                    applied_single_swaps.add(net2_name)
                    swap_pair_count += 1
                    total_layer_swaps += 2
                    print(f"  Swap pair: {net1_name} ({tgt1}->{src1}) + {net2_name} ({src2}->{src1})")
                    break

    # PHASE 2: Solo switches for remaining nets
