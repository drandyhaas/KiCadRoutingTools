...
def parse_voltage_from_net_name(net_name: str) -> Optional[float]:
    """
    Parse a voltage value from a power net name.
    ...
    - "VCC_3V3" -> 3.3V
    - "PWR_5V0" -> 5.0V
    - "/xilinx/+3,3V_OUT" -> 3.3V

    Returns None if no voltage can be extracted.
    """
    import re
    ...
    # Normalize: uppercase, replace comma with dot
    name = net_name.upper().replace(',', '.')

    # Pattern 1: Explicit voltage like "+3.3V", "-12V", "5V", "+3.3V_OUT"
    # Allow optional suffix after V (like _OUT)
    match = re.search(r'[+-]?(\d+\.?\d*)\s*V(?:OLTS?)?(?:[_\b]|$)', name)
    if match:
        return float(match.group(1))

    # Pattern 2: Voltage embedded in name like "VCC_3V3", "PWR_5V0"
    match = re.search(r'(\d+)V(\d+)', name)
    if match:
        whole = int(match.group(1))
        frac = int(match.group(2))
        return whole + frac / (10 ** len(match.group(2)))

    return None


def detect_max_voltage_from_nets(pcb_data: PCBData, power_net_ids: Set[int]) -> float:
    """
    Detect the maximum voltage used in the design from power net names.

    Examines the names of power nets to find voltage values (e.g., "+3.3V", "+5V")
    and returns the maximum voltage found, with a minimum of 5.0V for safety.

    Args:
        pcb_data: PCB data with net information
        power_net_ids: Set of net IDs identified as power nets

    Returns:
        Maximum voltage found in volts (minimum 5.0V)
    """
    max_voltage = 0.0

    for net_id in power_net_ids:
    ...
    # Ensure minimum 5V for safety margin
    return max(max_voltage, 5.0)


def parse_resistor_value(value_str: str) -> Optional[float]:
    """
    Parse a resistor value string and return resistance in ohms.

    Handles common formats:
    - "100" -> 100 ohms
    - "10K" or "10k" -> 10000 ohms
    - "4.7K" or "4,7K" -> 4700 ohms
    - "4K7" -> 4700 ohms (European notation)
    - "4R7" -> 4.7 ohms (European notation for < 10 ohms)
    - "0" or "0R" -> 0 ohms (jumper)
    - "1M" -> 1000000 ohms
    - "2.2M" -> 2200000 ohms

    Returns None if the value cannot be parsed as a resistor.
    """
    import re
    ...
    # Normalize: uppercase, replace comma with dot
    val = value_str.upper().strip().replace(',', '.')

    # Handle "0" or "0R" (zero-ohm jumper)
    if val in ('0', '0R', '0E'):
        return 0.0

    # Multiplier suffixes
    multipliers = {'R': 1, 'E': 1, 'K': 1000, 'M': 1000000}

    # Pattern 1: European notation with letter as decimal point (4K7 = 4.7K, 4R7 = 4.7R)
    match = re.match(r'^(\d+)([REKM])(\d+)$', val)
    if match:
        whole = int(match.group(1))
        suffix = match.group(2)
        frac = match.group(3)
        mult = multipliers.get(suffix, 1)
        return (whole + int(frac) / (10 ** len(frac))) * mult

    # Pattern 2: Standard notation (100, 10K, 4.7K, 2.2M)
    match = re.match(r'^(\d+\.?\d*)([REKM]?)$', val)
    if match:
        num = float(match.group(1))
        suffix = match.group(2)
        mult = multipliers.get(suffix, 1) if suffix else 1
        return num * mult

    return None


def propagate_power_widths_through_passives(
    pcb_data: PCBData,
    power_net_widths: Dict[int, float],
    series_prefixes: Tuple[str, ...] = ('L', 'FB', 'F'),
    max_voltage: float = None,
    min_current: float = 0.1
) -> Dict[int, float]:
    """
    Propagate power net widths through series passive components.

    When a power net connects through a series element (inductor, ferrite bead,
    fuse, or low-value resistor), the net on the other side carries the same
    current and needs the same track width. This is common for:
    - +3.3V -> L101 (inductor) -> /VDDPLL
    - GND -> FB101 (ferrite bead) -> GNDA
    - VIN -> R_sense (0.1 ohm) -> load

    Resistors are included if their resistance is low enough that significant
    current could flow: I = V_max / R >= min_current. This excludes high-value
    pull-up/pull-down resistors while including current sense resistors, zero-ohm
    jumpers, and series protection resistors.

    The max voltage is auto-detected from power net names (e.g., "+3.3V", "+5V",
    "+12V") if not explicitly provided.

    This function iteratively propagates widths until no more changes occur,
    handling chains of series elements.

    Args:
        pcb_data: Parsed PCB data with footprint and pad information
        power_net_widths: Initial power net widths from identify_power_nets()
        series_prefixes: Component reference prefixes to always treat as series
                        elements. Default: L (inductors), FB (ferrite beads),
                        F (fuses). Resistors (R) are handled separately.
        max_voltage: Maximum voltage for current calculation. If None (default),
                    auto-detected from power net names (e.g., "+3.3V" -> 3.3V).
                    Used to determine if a resistor could carry significant current.
        min_current: Minimum current threshold in amps (default 0.1A = 100mA).
                    Resistors where V_max/R >= min_current are included.

    Returns:
        Updated power_net_widths dict with propagated widths

    Example:
        # If +3.3V (net 104) has width 0.5mm and connects through L101 to
        # /VDDPLL (net 30), then net 30 will also get 0.5mm width.
        widths = identify_power_nets(pcb, ['*GND*', '+3.3V'], [0.5, 0.5])
        widths = propagate_power_widths_through_passives(pcb, widths)
    """
    # Make a copy to avoid modifying the input
    result = dict(power_net_widths)

    # Auto-detect max voltage from power net names if not provided
    if max_voltage is None:
        max_voltage = detect_max_voltage_from_nets(pcb_data, set(power_net_widths.keys()))
        print(f"Power propagation: detected max voltage {max_voltage}V from power net names")

    # Calculate max resistance for current threshold: R_max = V_max / I_min
    max_resistance = max_voltage / min_current if min_current > 0 else float('inf')

    # Build a map of 2-pin series elements: component_ref -> [(pad1_net_id, pad2_net_id)]
    series_connections: Dict[str, List[Tuple[int, int]]] = {}

    def matches_series_prefix(ref: str) -> bool:
        """Check if ref matches a series prefix (prefix followed by digit)."""
        for prefix in series_prefixes:
            if ref.startswith(prefix) and len(ref) > len(prefix):
                # Next char must be a digit (e.g., L101 not LED101)
                if ref[len(prefix)].isdigit():
                    return True
        return False

    def is_low_value_resistor(ref: str, value: str) -> bool:
        """Check if this is a resistor with low enough value to carry power current."""
        # Must be R followed by digit (R101, not RCAN201)
        if not (ref.startswith('R') and len(ref) > 1 and ref[1].isdigit()):
            return False

        resistance = parse_resistor_value(value)
        if resistance is None:
            return False

        # Zero-ohm resistors are always jumpers
        if resistance == 0:
            return True

        # Include if potential current >= threshold
        return resistance <= max_resistance

    for ref, fp in pcb_data.footprints.items():
        # Only consider 2-pin components (series elements that carry current through)
        if len(fp.pads) != 2:
            continue

        # Check if this is a series power component
        is_series = matches_series_prefix(ref) or is_low_value_resistor(ref, fp.value)
        if not is_series:
            continue

        # Get the two net IDs
        net_ids = [pad.net_id for pad in fp.pads if pad.net_id != 0]
    ...
    # Iteratively propagate until no changes
    changed = True
    iterations = 0
    ...
    while changed and iterations < max_iterations:
        ...
        for ref, connections in series_connections.items():
            for net_a, net_b in connections:
                width_a = result.get(net_a)
                width_b = result.get(net_b)

                if width_a is not None and width_b is None:
                    # Propagate from A to B
                    result[net_b] = width_a
                    changed = True
                elif width_b is not None and width_a is None:
                    # Propagate from B to A
                    result[net_a] = width_b
                    changed = True
                elif width_a is not None and width_b is not None:
                    # Both have widths - use the larger one for both
                    max_width = max(width_a, width_b)
                    if result[net_a] < max_width:
                        result[net_a] = max_width
                        changed = True
                    if result[net_b] < max_width:
                        result[net_b] = max_width
                        changed = True

    # Report what was propagated
    propagated = set(result.keys()) - set(power_net_widths.keys())
    if propagated:
        propagated_names = []
        for net_id in propagated:
            if net_id in pcb_data.nets:
                propagated_names.append(pcb_data.nets[net_id].name)
            else:
                propagated_names.append(f"net_{net_id}")
        print(f"Power width propagation: {len(propagated)} additional nets through passives: {', '.join(propagated_names)}")
