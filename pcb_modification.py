...
def chord_short_jogs(results, pcb_data: PCBData, scope_net_ids=None) -> int:
    """Chord (hypotenuse-fill) same-net short 135-deg jogs so KiCad's
    connection-width checker cannot flag their tangency micro-voids.

    A grid router reaching a target one cell off-axis emits a two-segment jog
    (straight + 45-deg diagonal, each about one track-width long). Grid
    arithmetic routinely lands such a jog's outer cap EXACTLY tangent to
    adjacent same-net copper (offset = half-width + half-width); whether the
    resulting copper union has a micro-void there is then decided by float
    dust, and KiCad's connection_width checker can report a sub-minimum web
    around the void (measured live: 0.079 mm on a jog that is provably
    >= 0.2 mm wide everywhere by polygon erosion). Rather than detect the
    tangency (the detector would inherit the same float-borderline), chord
    EVERY short jog: one segment across the jog's far endpoints, at the
    narrower arm's width. The chord triangle of a 135-deg jog with arms
    <= 1.5 track-widths is a sliver whose interior cannot legally contain
    foreign copper (no clearance-legal room that close to both arms), so the
    chord cannot create a clearance violation by construction. Additive only.

    Only jogs with at least one THIS-RUN arm are chorded (untouched input
    geometry is the input author's business). Returns the number of chords
    added; new segments join the write-list as a 'jog_chord' cleanup result.
    """
    import math
    from collections import defaultdict
    ...
    # arms[(net, layer, vertex)] = [(ux, uy, length, width, seg, far_end)]
    ...
    for s in pcb_data.segments:
        ...
        arms[(s.net_id, s.layer, rk(*a))].append((ux, uy, length, s.width, s, b))
        arms[(s.net_id, s.layer, rk(*b))].append((-ux, -uy, length, s.width, s, a))
        seg_keys.add((s.net_id, s.layer, min(rk(*a), rk(*b)), max(rk(*a), rk(*b))))

    new_chords = []
    COS135 = math.cos(math.radians(135.0))
    for (net_id, layer, _vk), lst in sorted(
            arms.items(), key=lambda kv: (kv[0][0], kv[0][1], kv[0][2])):
        if len(lst) < 2:
            continue
        for i in range(len(lst)):
            for j in range(i + 1, len(lst)):
                (ax, ay, alen, aw, aseg, afar) = lst[i]
                (bx, by, blen, bw, bseg, bfar) = lst[j]
                dot = ax * bx + ay * by
                if not (COS135 - 0.02 < dot < COS135 + 0.02):
                    continue
                w = min(aw, bw)
                if alen > 1.5 * w or blen > 1.5 * w:
                    continue  # a real elbow, not a jog
                if id(aseg) not in routed_seg_ids and id(bseg) not in routed_seg_ids:
                    continue  # purely pre-existing input jog: leave it
                key = (net_id, layer, min(rk(*afar), rk(*bfar)),
                       max(rk(*afar), rk(*bfar)))
                if key in seg_keys:
                    continue  # the hypotenuse already exists as copper
                seg_keys.add(key)
                g = Segment(start_x=afar[0], start_y=afar[1],
                            end_x=bfar[0], end_y=bfar[1],
                            width=w, layer=layer, net_id=net_id)
                new_chords.append(g)

    if new_chords:
        pcb_data.segments.extend(new_chords)
        results.append({'new_segments': new_chords, 'new_vias': [],
                        'cleanup': 'jog_chord'})
    return len(new_chords)
