...
# ---------------------------------------------------------------------------
# Plane-net manifest sidecar (chain consistency)
# ---------------------------------------------------------------------------
# The plane steps DECLARE which nets they own by writing their JSON summary to
# a sidecar `<output>_planes.json` next to the output board. Downstream steps
# (route/route_diff/fanout) read the sidecar next to their INPUT board and
# auto-exclude those nets from WILDCARD net selection -- so a plane net can
# never be fanned out or routed as a normal signal by a later chain step just
# because a '*' pattern matched it (the ottercast +1V1/VCC-DDR fanout-stub
# clutter class). An EXPLICIT (non-wildcard) pattern still includes the net:
# naming a plane net verbatim is an intentional override. Every step that
# writes an output board carries the manifest forward (carry_plane_manifest),
# so the declaration survives the whole chain; copy_board.py copies it as a
# sibling automatically.

def plane_manifest_path(board_path: str) -> str:
    import os
    return os.path.splitext(board_path)[0] + '_planes.json'


def read_plane_manifest(board_path: str) -> dict:
    """Manifest next to `board_path`, or {} when absent/unreadable."""
    import os, json
    p = plane_manifest_path(board_path)
...
def record_plane_manifest(output_board_path: str, plane_nets, script: str,
                          input_board_path: str = None, summary: dict = None) -> dict:
    """Write/merge the plane manifest next to `output_board_path`.

    Unions `plane_nets` with any manifest already next to the OUTPUT (a
    re-run) and, when `input_board_path` is given, with the manifest next to
    the INPUT (an earlier plane step) -- chained plane steps accumulate.
    Appends a step record carrying the step's own JSON `summary`."""
    import json
    m = read_plane_manifest(output_board_path)
    if input_board_path:
        prior = read_plane_manifest(input_board_path)
        m.setdefault('plane_nets', [])
        m['plane_nets'] = sorted(set(m.get('plane_nets', []))
                                 | set(prior.get('plane_nets', [])))
        m.setdefault('steps', []).extend(
            s for s in prior.get('steps', []) if s not in m.get('steps', []))
    nets = sorted(set(m.get('plane_nets', [])) | {n for n in plane_nets if n})
    m['plane_nets'] = nets
    m.setdefault('steps', []).append(
        {'script': script, 'nets': sorted({n for n in plane_nets if n}),
         **({'summary': summary} if summary else {})})
    with open(plane_manifest_path(output_board_path), 'w', encoding='utf-8') as f:
        json.dump(m, f, indent=1)
    return m


def carry_plane_manifest(input_board_path: str, output_board_path: str) -> bool:
    """Propagate the input board's manifest to the output board (non-plane
    steps call this after writing their output). Returns True if carried."""
    import json
    m = read_plane_manifest(input_board_path)
    if not m.get('plane_nets'):
        return False
    out = read_plane_manifest(output_board_path)
    merged_nets = sorted(set(m.get('plane_nets', [])) | set(out.get('plane_nets', [])))
    merged = dict(m)
    merged['plane_nets'] = merged_nets
    if out.get('steps'):
        merged['steps'] = out['steps'] + [s for s in m.get('steps', [])
                                          if s not in out['steps']]
    with open(plane_manifest_path(output_board_path), 'w', encoding='utf-8') as f:
        json.dump(merged, f, indent=1)
    return True


def apply_plane_manifest_exclusions(board_path: str, patterns, selected_names):
    """Drop manifest-declared plane nets from a WILDCARD-matched selection.

    `selected_names` is the already-expanded net list; a net is dropped iff it
    is in the input board's plane manifest AND no pattern names it verbatim
    (explicit inclusion wins). Returns (filtered_names, excluded_names)."""
    m = read_plane_manifest(board_path)
    plane = set(m.get('plane_nets', []))
    if not plane:
        return list(selected_names), []
    literal = {p for p in (patterns or []) if p in plane}
    excluded = [n for n in selected_names if n in plane and n not in literal]
    if excluded:
        kept = [n for n in selected_names if n not in set(excluded)]
