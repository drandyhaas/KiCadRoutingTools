...
ROWS = [
    ...
    ('instrument-not-named', 'mp',
     ...
     "        'legality': (TITLES['legality'], leg_l, ['legality']),",
     (T1042,), 'KILLED'),
    ...
    ('nothing-moved-reads-as-a-placement', 'mp',
     ...
     "        return True",
     (T1042,), 'KILLED'),
    ('measured-before-the-cheap-gate', 'mp',
     "    if len(cands) < 2:",
     "    if [measure_board(b, cache) for _l, b in cands] and len(cands) < 2:",
     (T1042,), 'KILLED'),
    ('a-subprocess-in-the-measurement', 'mp',
     "            pcb = parse_kicad_pcb(path)",
     "            import subprocess; subprocess.run([sys.executable, '-c', '0']); pcb = parse_kicad_pcb(path)",
     (T1042,), 'KILLED'),
    ...
    ('ledger-floorplan-ignored', 'mp',
     ...
     "        elif False:",
     (T1042,), 'KILLED'),
    ('two-instruments-on-one-line', 'mp',
     "            fpv, why = check_floorplan_errors(b, intent)",
     "            fpv, why = (lfp, '') if lfp is not None else check_floorplan_errors(b, intent)",
     (T1042,), 'KILLED'),
    ('unmeasured-plotted-as-zero', 'mp',
     "                res = {'unmeasured': 'no parts on the board'}",
     "                res = {'off_outline': 0, 'conflict_pairs': 0, 'overlap_mm2': 0.0, 'crossings': 0, 'hpwl': 0.0, 'locked_pairs': 0}",
     (T1042,), 'KILLED'),
    ('locked-floor-unlabelled', 'mp',
     "        leg_l.append((ROLE['floor'], 'floor %s = locked parts' % _fmt(floor),",
     "        leg_l.append((ROLE['floor'], 'floor %s' % _fmt(floor),",
     (T1042,), 'KILLED'),
    ('plot-floor-dropped', 'mp',
     "PLOT_MIN_PX = 48",
     "PLOT_MIN_PX = 12",
     (T1042,), 'KILLED'),
    ('dropped-panels-unsaid', 'mp',
     "    if dropped:",
     "    if False:",
     (T1042,), 'KILLED'),
    ('run-time-ignored', 'mp',
     ...
     "        t0, t1 = 0.0, 1e9",
     (T1042,), 'KILLED'),
    ('beats-share-a-pixel', 'mp',
     "            xs[i] = max(xs[i], xs[i - 1] + MIN_BEAT_PX)",
     "            xs[i] = xs[i]",
     (T1042,), 'KILLED'),
    ('glide-shows-the-destination', 'mp',
     "        return lands.get(nb, firsts.get(nb, b.first))",
     "        return firsts.get(nb, b.first)",
     (T1042,), 'KILLED'),
    ('flags-overprint', 'mp',
     "        taken[lv].append(span)",
     "        lv = 0",
     (T1042,), 'KILLED'),
    ('marker-in-the-intent-gold', 'mp',
