...
ROWS = [
    ...
    ('measured-row-of-any-ref-counts', 'fp',
     ...
     "        if any(row.get('declared')\n",
     (T974U,), 'KILLED'),
    ('measured-row-of-an-undeclared-entry-counts', 'fp',
     "        if any(str(row.get('ref')) == ref and row.get('declared')\n",
     "        if any(str(row.get('ref')) == ref\n",
     (T974U,), 'KILLED'),
    # The edge equality also keeps an EDGELESS entry from borrowing a row --
    # every row names its edge -- which is why the explicit `edge is not
    # None` guard this once carried was removed: the battery measured it
    # inert.
    ...
    ('abstention-outranks-a-measured-row', 'fp',
     ...
     "graded.budget_abstained and any(str(row.get('ref')) == ref and "
     "row.get('declared')\n",
     (T974U,), 'KILLED'),
    ...
    ('evidence-emptied', 'fp',
     ...
     "        'overhang_evidence': [],",
     (T974U,), 'KILLED'),
    ('evidence-carries-the-per-pad-lists', 'fp',
     "            slim_copper['n_' + name] = len(copper.get(name) or ())",
     "            slim_copper[name] = copper.get(name)",
     (T974U,), 'KILLED'),
    ('evidence-counts-read-zero', 'fp',
     "            slim_copper['n_' + name] = len(copper.get(name) or ())",
     "            slim_copper['n_' + name] = 0",
     (T974U,), 'KILLED'),
