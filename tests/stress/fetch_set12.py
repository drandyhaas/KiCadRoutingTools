#!/usr/bin/env python3
"""Fetch set12 .kicad_pcb sources listed in manifest_set12.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set12/.
After fetching, run `bash prep_set12.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set12/ (routed reference) +
boards_unrouted_set12/ (stripped).

  python3 fetch_set12.py
  bash prep_set12.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set12.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set12"
    ...
    print(f"\n{ok}/{len(boards)} set12 sources -> {out_dir}")
