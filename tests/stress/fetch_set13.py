#!/usr/bin/env python3
"""Fetch set13 .kicad_pcb sources listed in manifest_set13.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set13/.
After fetching, run `bash prep_set13.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set13/ (routed reference) +
boards_unrouted_set13/ (stripped).

  python3 fetch_set13.py
  bash prep_set13.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set13.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set13"
    ...
    print(f"\n{ok}/{len(boards)} set13 sources -> {out_dir}")
