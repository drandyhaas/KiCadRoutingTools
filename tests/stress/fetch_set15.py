#!/usr/bin/env python3
"""Fetch set15 .kicad_pcb sources listed in manifest_set15.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set15/.
After fetching, run `bash prep_set15.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set15/ (routed reference) +
boards_unrouted_set15/ (stripped).

  python3 fetch_set15.py
  bash prep_set15.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set15.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set15"
    ...
    print(f"\n{ok}/{len(boards)} set15 sources -> {out_dir}")
