#!/usr/bin/env python3
"""Fetch set16 .kicad_pcb sources listed in manifest_set16.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set16/.
After fetching, run `bash prep_set16.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set16/ (routed reference) +
boards_unrouted_set16/ (stripped).

  python3 fetch_set16.py
  bash prep_set16.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set16.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set16"
    ...
    print(f"\n{ok}/{len(boards)} set16 sources -> {out_dir}")
