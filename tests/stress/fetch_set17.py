#!/usr/bin/env python3
"""Fetch set17 .kicad_pcb sources listed in manifest_set17.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set17/.
After fetching, run `bash prep_set17.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set17/ (routed reference) +
boards_unrouted_set17/ (stripped).

  python3 fetch_set17.py
  bash prep_set17.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set17.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set17"
    ...
    print(f"\n{ok}/{len(boards)} set17 sources -> {out_dir}")
