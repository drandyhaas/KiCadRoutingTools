#!/usr/bin/env python3
"""Fetch set18 .kicad_pcb sources listed in manifest_set18.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set18/.
After fetching, run `bash prep_set18.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set18/ (routed reference) +
boards_unrouted_set18/ (stripped).

  python3 fetch_set18.py
  bash prep_set18.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set18.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set18"
    ...
    print(f"\n{ok}/{len(boards)} set18 sources -> {out_dir}")
