#!/usr/bin/env python3
"""Fetch set20 .kicad_pcb sources listed in manifest_set20.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set20/.
After fetching, run `bash prep_set20.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set20/ (routed reference) +
boards_unrouted_set20/ (stripped).

  python3 fetch_set20.py
  bash prep_set20.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set20.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set20"
    ...
    print(f"\n{ok}/{len(boards)} set20 sources -> {out_dir}")
