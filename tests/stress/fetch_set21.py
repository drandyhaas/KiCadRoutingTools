#!/usr/bin/env python3
"""Fetch set21 .kicad_pcb sources listed in manifest_set21.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set21/.
After fetching, run `bash prep_set21.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set21/ (routed reference) +
boards_unrouted_set21/ (stripped).

  python3 fetch_set21.py
  bash prep_set21.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set21.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set21"
    ...
    print(f"\n{ok}/{len(boards)} set21 sources -> {out_dir}")
