#!/usr/bin/env python3
"""Fetch set22 .kicad_pcb sources listed in manifest_set22.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set22/.
After fetching, run `bash prep_set22.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set22/ (routed reference) +
boards_unrouted_set22/ (stripped).

  python3 fetch_set22.py
  bash prep_set22.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set22.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set22"
    ...
    print(f"\n{ok}/{len(boards)} set22 sources -> {out_dir}")
