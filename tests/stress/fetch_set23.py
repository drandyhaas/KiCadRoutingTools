#!/usr/bin/env python3
"""Fetch set23 .kicad_pcb sources listed in manifest_set23.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set23/.
After fetching, run `bash prep_set23.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set23/ (routed reference) +
boards_unrouted_set23/ (stripped).

  python3 fetch_set23.py
  bash prep_set23.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set23.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set23"
    ...
    print(f"\n{ok}/{len(boards)} set23 sources -> {out_dir}")
