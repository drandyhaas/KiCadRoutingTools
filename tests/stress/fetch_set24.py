#!/usr/bin/env python3
"""Fetch set24 .kicad_pcb sources listed in manifest_set24.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set24/.
After fetching, run `bash prep_set24.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set24/ (routed reference) +
boards_unrouted_set24/ (stripped).

  python3 fetch_set24.py
  bash prep_set24.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set24.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set24"
    ...
    print(f"\n{ok}/{len(boards)} set24 sources -> {out_dir}")
