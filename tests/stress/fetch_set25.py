#!/usr/bin/env python3
"""Fetch set25 .kicad_pcb sources listed in manifest_set25.json (raw download).

Mirrors fetch_set10.py. Downloads each board (and its sibling .kicad_pro, which
carries the DRC floor) into $STRESS_DIR/sources/github_set25/.
After fetching, run `bash prep_set25.sh` (needs KiCad's bundled python /
pcbnew) to produce boards_set25/ (routed reference) +
boards_unrouted_set25/ (stripped).

  python3 fetch_set25.py
  bash prep_set25.sh
"""
import json
import os
...
MANIFEST = HERE / "manifest_set25.json"


def main():
    boards = json.loads(MANIFEST.read_text())
    out_dir = STRESS / "sources/github_set25"
    ...
    print(f"\n{ok}/{len(boards)} set25 sources -> {out_dir}")
