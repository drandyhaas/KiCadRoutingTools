...
# The GUI front's oracle site. On main this is
# gui_utils.run_kicad_oracle_on_live_board; the IPC port has no SWIG
# live-board helper, so the same leg lives in routing_dialog (calling
# kicad_ipc_adapter.apply_oracle_reconnect). Read BOTH and require the
# check in whichever one this branch has, rather than pinning a filename
# the port renamed -- and assert the scan found a real file, because an
# unreadable path here would read exactly like a clean result.
_gu = ""
_gu_seen = []
for _cand in (('kicad_routing_plugin', 'gui_utils.py'),
              ('kicad_routing_plugin', 'routing_dialog.py'),
              ('kicad_ipc_adapter.py',)):
    _p = os.path.join(ROOT, *_cand)
    if os.path.isfile(_p):
        _gu += open(_p, encoding='utf-8').read()
        _gu_seen.append(os.path.join(*_cand))
assert _gu_seen, "no GUI oracle front found to scan -- the check would be vacuous"
