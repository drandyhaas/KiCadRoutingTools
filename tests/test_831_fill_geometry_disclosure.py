...
check("_live_fill publishes last_status in its finally (covers every return)",
      _in_finally)
# The count this used to assert (>= 4) was a PROXY for "the account covers
# every way out", and it was tuned to the SWIG front, which has a fourth
# return from #828's do-not-retry-a-thrown-SaveBoard path. The IPC front
# marshals nothing to a UI thread, so that path does not exist here and the
# count is legitimately 3 -- adding a return to satisfy a number would be
# gaming the gate.
#
# Assert the PROPERTY instead, which is both branch-independent and stronger
# than the count: every `return` in _live_fill sits lexically inside the try
# whose finally publishes last_status, so the account covers all of them
# however many there are. A return moved out from under that try is exactly
# the regression the count was groping for, and this catches it.
_returns = [n for n in ast.walk(_lf) if isinstance(n, ast.Return)]
check("_live_fill has return paths at all (a provider that cannot return is "
      "not a provider)", len(_returns) >= 1, len(_returns))
# A return is covered when control cannot reach it without passing through
# the finally. Two shapes qualify: INSIDE the try, and AFTER the whole try
# statement (reachable only by falling out of it, which runs the finally
# first). The one that would NOT be covered is a return that precedes the
# try -- an early exit bolted onto the top of the function, which is exactly
# how a provider comes to answer without leaving an account.
_inside = {id(n) for n in ast.walk(_try) if isinstance(n, ast.Return)} if _try else set()
_uncovered = [n.lineno for n in _returns
              if id(n) not in _inside and _try is not None
              and n.lineno < _try.lineno]
check("no _live_fill return escapes the finally that publishes the account",
      not _uncovered,
      f"return(s) at line(s) {_uncovered} run BEFORE the try, so they leave "
      f"no account behind")
