...
It self-skips (exit 77 with a SKIP: line) when the ledger is absent. Point it at
any teed run's ledger with:

    KRT_CMD_TIMING_LEDGER=/path/to/cmd_timing.jsonl python3 tests/test_887_run24_regression.py
...
def test_the_ledger_rows_are_disjoint_in_time():
    ...
    overlaps = [(a['label'], b['label'])
                ...
                if a['t_end'] is not None and b['t_start'] is not None
