...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # run_drc returns its ACCEPTED rows too (#962: a filled+capped via in
        # a paste opening is published as `protected-via-in-paste`), so clean
        # means no COUNTED row, which is what check_drc's own exit status reads.
        def _counted(path):
